...
class LayerClient(LayerCommon):
    ...
    def setup(self, **kwargs):
        ...
        if 'is_input_layer' in kwargs:
            assert isinstance(kwargs['is_input_layer'], bool)
            self.is_input_layer = kwargs['is_input_layer']
        if 'is_output_layer' in kwargs:
    ...
    def offline(self) -> None:
        t = time.time()
        # print("r", self.r)
        self.send_offline(self.r)
        self.pre = self.recv_offline()
        # print("pre", self.pre)
        self.stat.time_offline += time.time() - t
    ...
    def online(self, xmdual) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        self.send_online(xmdual[0], xmdual[1])
        data = self.recv_online()
        # print("wxm", data)
