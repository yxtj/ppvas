...
def train(data_dir, chkpt_dir, model, lr=0.001, epochs=100, batch_size=512, device='cpu'):
    ...
    while i < epochs:
        ...
        util.train(model, trainset, batch_size, interval,
                   optimizer=optimizer, loss_fn=loss_fn, show_interval=30, device='cuda')
        file = chkpt_dir + "/minionn_" + str(i) + ".pt"
...
def test(data_dir, model, batch_size, device):
    ...
    util.test(model, testset, batch_size, device)
...
if __name__ == '__main__':
    ...
    file = util.find_latest_model(chkpt_dir, 'minionn_')
    if file is None:
    ...
    train(data_dir, chkpt_dir, model, lr, epochs, batch_size, device)
